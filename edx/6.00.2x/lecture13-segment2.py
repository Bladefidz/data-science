...
for t in range(numTrials):
	...
	# # Wrong statistical conclusion taken by US attorney to show location 111
	# # has 143 cases (greater from expected number of cases).
	# if max(locs) >= 143:
	# 	numGreater += 1

	# To fix that, we need to do a monte carlo simulation to proof that the
	# probability of location 111 has 143 cases is very significant.
	if loc[111] >= 143:
		numGreater += 1
prob = round(numGreater/numTrials, 4)
# print('Est. probability of at least one region having \
# at least 143 cases =', prob)
print('Est. probability of location 111 having \
at least 143 cases =', prob)
